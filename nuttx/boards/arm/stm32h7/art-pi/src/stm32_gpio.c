...
static int gpint_attach(struct gpio_dev_s *dev, pin_interrupt_t callback) {
	...
	stm32_gpiosetevent(g_gpiointinputs[stm32gpint->stm32gpio.id], false, false, false,
					   NULL, NULL);
...
static int gpint_enable(struct gpio_dev_s *dev, bool enable) {
	...
	if (enable) {
		if (stm32gpint->callback != NULL) {
			...
			stm32_gpiosetevent(g_gpiointinputs[stm32gpint->stm32gpio.id], true, false,
							   false, stm32gpio_interrupt,
